...
import (
	...
	legacy "github.com/haolipeng/LLM-Scope/internal/bpf/stdiocap"
)

// 将 bpf2go 生成的 internal/bpf/stdiocap 类型与加载函数导出为 runtime 包 API。
type (
	Objects        = legacy.Objects
	Programs       = legacy.Programs
	Maps           = legacy.Maps
	Variables      = legacy.Variables
	StdiocapEventT = legacy.StdiocapEventT
)
