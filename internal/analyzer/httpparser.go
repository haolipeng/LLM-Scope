...
func (p *HTTPParser) Process(ctx context.Context, in <-chan *core.Event) <-chan *core.Event {
...
func (p *HTTPParser) parseEvent(event *core.Event) *core.Event {
	...
	httpEvent := buildHTTPEvent(parsed, tid, event, p.includeRaw)
	return httpEvent
}
...
func buildHTTPEvent(msg *httpMessage, tid uint64, original *core.Event, includeRaw bool) *core.Event {
